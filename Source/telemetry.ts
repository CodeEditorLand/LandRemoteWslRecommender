...
export function enableTelemetry(): boolean {
	return (
		vscode.workspace.getConfiguration().get(ConfigKeys.enableTelementry) !==
		false
	);
}
...
export function enableExperiments(): boolean {
	return (
		vscode.workspace
			.getConfiguration()
			.get(ConfigKeys.enableExperiments) !== false
	);
}
...
export function enableAllExperimentalFeatures(): boolean {
	return (
		vscode.workspace
			.getConfiguration()
			.get(ConfigKeys.enableAllExperiments) === true
	);
}
...
export function getTelemetry(
	...
	const initExperimentationService = () => {
		if (enableExperiments() && !enableAllExperimentalFeatures()) {
			...
			*/
			experimentService = getExperimentationService(
				`${publisher}.${name}`,
				version,
				target,
				reporter,
	...
	context.subscriptions.push(
		vscode.workspace.onDidChangeConfiguration((e) => {
			if (
				e.affectsConfiguration(ConfigKeys.enableTelementry) ||
				e.affectsConfiguration(ConfigKeys.enableExperiments) ||
	...
	telemetry = {
		...
		async isExperimentEnabled(experiment: Experiment): Promise<boolean> {
			...
			return (
				experimentService !== undefined &&
				experimentService.isCachedFlightEnabled(experiment)
			);
		},
