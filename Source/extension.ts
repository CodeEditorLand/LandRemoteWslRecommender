...
import { Command, Dialog, Experiment, getTelemetry } from "./telemetry";
...
const WIN10_1903 = {
	label: "Windows 10, May 2019 Update, version 1903",
	build: 18362,
};
...
export async function activate(context: vscode.ExtensionContext) {
	if (
		...
		getWindowsBuildNumber() < WIN10_1903.build ||
	...
	async function addFeature(
		...
		initFeature: () => vscode.Disposable,
	) {
		let featureDisposable: vscode.Disposable | undefined = undefined;

		const isFeatureEnabled = async () => {
			return (
				!vscode.extensions.getExtension(ExtensionId.remoteWSL) &&
				(await telemetry.isExperimentEnabled(experiment))
			);
		};
			...
			async () => {
				...
				if (isWSLInstalled !== true) {
					...
					const response = await vscode.window.showErrorMessage(
						localize(
							"installWSL",
							"The Windows Subsystem for Linux (WSL) must be installed to complete the action. WSL lets you run a GNU/Linux environment directly on Windows without the overhead of a traditional virtual machine. VS Code, through the WSL extension, can then open folders and run commands, extensions, and the terminal in WSL.",
						),
						...buttons,
					);
					...
					const res = await vscode.window.showInformationMessage(
						localize(
							...
							"The 'WSL' extension is required to complete the action. The extension allows to open a window where commands, extensions and the terminal run in the Linux subsystem.",
						),
						showExtension,
					);
	...
	addFeature(
		Experiment.openWSLDocumentation,
		ContextKey.gettingStarted,
		() => {
			return vscode.commands.registerCommand(
